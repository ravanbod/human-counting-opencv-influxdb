...
def detect(frame):
    ...
    influx_client = InfluxDBClient(
        url="http://3.72.82.157:8086", token=token, org=org)
    print(influx_client.ping())
    influx_write_api = influx_client.write_api(write_options=SYNCHRONOUS)
    influx_write_api.write(bucket_name, org, ["persons,location=door1 persons={}".format(
        person-1+(random.randint(0, 5))).encode()])
    print("persons", person-1, time.localtime())
